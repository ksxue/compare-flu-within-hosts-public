...
clean="$8" # If 0, reruns entire pipeline and regenerates all intermediates.

# Other paths.
SummarizeBAM="bin/SummarizeBAM-1.21"
AnnotateVariants="bin/AnnotateVariants-1.11"
...
then
  ...
  bowtie2-build ${dir}/${sample}.fasta ${dir}/${sample}
  # Align reads.
...
# Do not perform deduplication step from paired-end pipeline.
:<<END
...
if [ ! -f ${dir}/${sample}-realigned.summary ] || [ ${clean} -eq "0" ];
then
  ${SummarizeBAM} -i <(samtools view ${dir}/${sample}-realigned.bam) \
    -f ${dir}/${sample}.fasta -o ${dir}/${sample}-realigned.summary
fi
...
then
  ${AnnotateVariants} -i ${dir}/${sample}-realigned.summary \
    -f ${dir}/${sample}.fasta -b ${bedfile} -o ${dir}/${sample}-realigned-annotated.summary
